...
function bound(x){
  if (x >= GAME_WIDTH-STATE.jet_width){
    STATE.x_pos = GAME_WIDTH-STATE.jet_width;
    return GAME_WIDTH-STATE.jet_width
  } if (x <= 0){
    STATE.x_pos = 0;
    return 0
  } else {
...
function createPlayer($container) {
  ...
  $player.src = "img/F22 jet-258913.png";
  $player.className = "player";
  $container.appendChild($player);
  setPosition($player, STATE.x_pos, STATE.y_pos);
  setSize($player, STATE.jet_width);
}
...
function updatePlayer(){
  if(STATE.move_left){
    ...
    createLaser($container, STATE.x_pos - STATE.jet_width/2, STATE.y_pos);
    STATE.cooldown = 10;
  }
